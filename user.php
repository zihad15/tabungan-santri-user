		...
		if (count($user) > 0) {
			...
			foreach ($user as $row) {
				...
				if (count($finger) == 0) {
					...
					$verification = "<a href='finspot:FingerspotVer;$url_verification' class='btn btn-xs btn-success' id='myBtn'>Login</a>";
...
<?php
		...
		if (count($res) > 1) {
			...
			$sql 	= "INSERT INTO user SET username='".$_GET['user_name']."' ";
